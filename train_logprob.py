...
from utils.LogprobGRPOTrainer import *

max_seq_length = 1024 # Can increase for longer reasoning traces
lora_rank = 64 # Larger rank = smarter, but slower

save_name = "saved_1_1000_step_logprob_scale_2"
...
def logprob_reward_func(prompts, completions, answer, **kwargs) -> list[float]:
    ...
    for prompt, completion, a_gold in zip(prompts, completions, answer):
        ...
        if "<answer>" not in gen or "</answer>" not in gen:
            rewards.append(float('-inf'))
            continue

        reasoning = gen.split("<answer>")[0]
        
        if '<reasoning>' not in gen or '</reasoning>' not in gen:
            rewards.append(float('-inf'))
            continue

        ctx_str  = prompt_text + reasoning
        full_str = ctx_str + '<answer>\n' + a_gold.strip() + '\n</answer>'
    ...
    return [math.exp(x) * 5.0 for x in rewards] 
